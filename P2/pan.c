...
short src_ln1 [] = {
	  0, 102, 104, 106, 108, 109, 110, 111, 
	113, 114, 114, 105, 117, 101, 118, 101, 
	118, 119,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "Pessimistic.pml", 1, 17 },
	{ "-", 18, 19 }
};
uchar reached1 [] = {
	  0,   1,   0,   1,   0,   0,   1,   1, 
...
short src_ln0 [] = {
	 ...
	 60,  53,  63,  53,  64,  65,  67,   0, 
	  0,   0,   0,  70,  76,  78,  79,  80, 
	 81,  82,  69,  84,  84,  88,  89,  90, 
	 66,  92,  63,  95,  63,  95,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "Pessimistic.pml", 1, 14 },
	{ "-", 15, 18 },
	{ "Pessimistic.pml", 19, 37 },
	{ "-", 38, 39 }
};
uchar reached0 [] = {
	  0,   1,   0,   0,   0,   0,   0,   1, 
...
char *procname[] = {
   ...
   "notAvailablePlusFlightFullEqualsNthTriangle",
   "alwaysEventuallyCustomersBuyingProcessDone",
   ":np_:",
	0
};
...
int Btypes[] = {
   ...
   1,	/* notAvailablePlusFlightFullEqualsNthTriangle */
   1,	/* alwaysEventuallyCustomersBuyingProcessDone */
   0	/* :np_: */
};
...
{	int j = 0, h = now._nr_pr;
...
#ifndef NOCLAIM
	case 8:	/* claim select */
		spin_c_typ[0] = 6; /* alwaysEventuallyCustomersBuyingProcessDone */
		((P8 *)pptr(h))->c_cur[0] = 3;
		reached6[3]=1;
		spin_c_typ[1] = 5; /* notAvailablePlusFlightFullEqualsNthTriangle */
		((P8 *)pptr(h))->c_cur[1] = 3;
		reached5[3]=1;
		spin_c_typ[2] = 4; /* ifMoreClientsThanSeatsFlightFullCounterGreaterThanZero */
		((P8 *)pptr(h))->c_cur[2] = 5;
		reached4[5]=1;
		spin_c_typ[3] = 3; /* allSeatsAlwaysAssigned */
		((P8 *)pptr(h))->c_cur[3] = 5;
		reached3[5]=1;
		((P8 *)pptr(h))->_t = 3;
		((P8 *)pptr(h))->_p = 5;
		((P8 *)pptr(h))->_n = 3; /* allSeatsAlwaysAssigned */
		src_claim = src_ln3;
#ifndef BFS
	...
	case 7:	/* np_ */
		...
		accpstate[7][1] = 1;
		break;
	case 6:	/* alwaysEventuallyCustomersBuyingProcessDone */
		((P6 *)pptr(h))->_t = 6;
		((P6 *)pptr(h))->_p = 3;
#ifdef HAS_PRIORITY
...
#endif
		break;
	case 5:	/* notAvailablePlusFlightFullEqualsNthTriangle */
		((P5 *)pptr(h))->_t = 5;
		((P5 *)pptr(h))->_p = 3;
#ifdef HAS_PRIORITY
	...
	case 1:	/* client */
		...
		reached1[13]=1;
		/* params: */
		((P1 *)pptr(h))->clientId = par0;
		/* locals: */
		((P1 *)pptr(h))->seatNumber = 0;
		((P1 *)pptr(h))->messageType = 0;
#ifdef VAR_RANGES
		logval("client:messageType", ((P1 *)pptr(h))->messageType);
		logval("client:clientId", ((P1 *)pptr(h))->clientId);
		logval("client:seatNumber", ((P1 *)pptr(h))->seatNumber);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* crs */
		...
		/* params: */
		/* locals: */
		((P0 *)pptr(h))->notAvailableSeatCount = 0;
		((P0 *)pptr(h))->flightFullMessageCount = 0;
		((P0 *)pptr(h))->assignedSeats = 0;
		((P0 *)pptr(h))->receivedCustomerId = 0;
		((P0 *)pptr(h))->receivedSeat = 0;
		((P0 *)pptr(h))->message = 0;
		((P0 *)pptr(h))->i = 0;
#ifdef VAR_RANGES
		logval("crs:message", ((P0 *)pptr(h))->message);
		logval("crs:notAvailableSeatCount", ((P0 *)pptr(h))->notAvailableSeatCount);
		logval("crs:flightFullMessageCount", ((P0 *)pptr(h))->flightFullMessageCount);
		logval("crs:assignedSeats", ((P0 *)pptr(h))->assignedSeats);
		logval("crs:receivedCustomerId", ((P0 *)pptr(h))->receivedCustomerId);
		logval("crs:receivedSeat", ((P0 *)pptr(h))->receivedSeat);
		logval("crs:i", ((P0 *)pptr(h))->i);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
	{	int l_in;
		...
		now.clientToCrs = addqueue(calling_pid, 1, 0);
		now.crsToClient[0] = addqueue(calling_pid, 2, 0);
		...
		now.seats[1].customerId = 0;
		now.seats[2].seatStatus = 0;
		now.seats[2].customerId = 0;
#ifdef VAR_RANGES
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
		...
		logval("seats[0].seatStatus", now.seats[0].seatStatus);
		logval("seats[0].customerId", now.seats[0].customerId);
		logval("seats[1].seatStatus", now.seats[1].seatStatus);
		logval("seats[1].customerId", now.seats[1].customerId);
		logval("seats[2].seatStatus", now.seats[2].seatStatus);
		logval("seats[2].customerId", now.seats[2].customerId);
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.clientToCrs) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.clientToCrs) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.clientToCrs) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.clientToCrs) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  EMPTY:	3\n");
	printf("	mtype  RESERVED:	2\n");
	printf("	mtype  ASSIGNED:	1\n");
	printf("	mtype  GET_SEAT:	10\n");
	printf("	mtype  RETURNING_SEAT:	9\n");
	printf("	mtype  PAYMENT_SEAT:	8\n");
	printf("	mtype  NOT_AVAILABLE_SEAT:	7\n");
	printf("	mtype  FLIGHT_FULL:	6\n");
	printf("	mtype  SUCCESS_SEAT:	5\n");
	printf("	mtype  INVALID_CUSTOMER:	4\n");
		printf("	(struct seats)\n");
	printf("	mtype  seatStatus:	%d\n", now.seats[ 0 ].seatStatus);
	printf("	int    customerId:	%d\n", now.seats[ 0 ].customerId);
	printf("	mtype  seatStatus:	%d\n", now.seats[ 1 ].seatStatus);
	printf("	int    customerId:	%d\n", now.seats[ 1 ].customerId);
	printf("	mtype  seatStatus:	%d\n", now.seats[ 2 ].seatStatus);
	printf("	int    customerId:	%d\n", now.seats[ 2 ].customerId);
	printf("	chan clientToCrs (=%d):	len %d:\t", now.clientToCrs, q_len(now.clientToCrs));
	c_chandump(now.clientToCrs);
	printf("	chan crsToClient[0] (=%d):	len %d:\t", now.crsToClient[0], q_len(now.crsToClient[0]));
	c_chandump(now.crsToClient[0]);
	printf("	chan crsToClient[1] (=%d):	len %d:\t", now.crsToClient[1], q_len(now.crsToClient[1]));
	c_chandump(now.crsToClient[1]);
	printf("	chan crsToClient[2] (=%d):	len %d:\t", now.crsToClient[2], q_len(now.crsToClient[2]));
...
{	/* int i; */
	...
	printf("	int    clientId:	%d\n", ((P1 *)pptr(pid))->clientId);
	printf("	int    seatNumber:	%d\n", ((P1 *)pptr(pid))->seatNumber);
	printf("	mtype  messageType:	%d\n", ((P1 *)pptr(pid))->messageType);
		break;
	case 0:
		printf("local vars proc %d (crs):\n", pid);
	printf("	int    notAvailableSeatCount:	%d\n", ((P0 *)pptr(pid))->notAvailableSeatCount);
	printf("	int    flightFullMessageCount:	%d\n", ((P0 *)pptr(pid))->flightFullMessageCount);
	printf("	int    assignedSeats:	%d\n", ((P0 *)pptr(pid))->assignedSeats);
	printf("	int    receivedCustomerId:	%d\n", ((P0 *)pptr(pid))->receivedCustomerId);
	printf("	int    receivedSeat:	%d\n", ((P0 *)pptr(pid))->receivedSeat);
	printf("	mtype  message:	%d\n", ((P0 *)pptr(pid))->message);
	printf("	int    i:	%d\n", ((P0 *)pptr(pid))->i);
		break;
	}
}
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("ASSIGNED"); return;
	case 2: Printf("RESERVED"); return;
	case 3: Printf("EMPTY"); return;
	case 4: Printf("INVALID_CUSTOMER"); return;
	case 5: Printf("SUCCESS_SEAT"); return;
	case 6: Printf("FLIGHT_FULL"); return;
	case 7: Printf("NOT_AVAILABLE_SEAT"); return;
	case 8: Printf("PAYMENT_SEAT"); return;
	case 9: Printf("RETURNING_SEAT"); return;
	case 10: Printf("GET_SEAT"); return;
